...
const colo26 = applySeason(data.schools.find((s) => s.id === 'colorado'), 2026)
ok(leadBookedNil(colo26).value === 7_672_052.88, 'colorado 2026 lead booked is the Sportico partial window')
ok(leadBookedNil(colo26).carry === true, 'colorado 2026 partial window is a Year 1 carry, not 2026–27 booked')
ok(leadBookedNil(colo26).field?.partialYear === true, 'colorado 2026 lead is labeled partial')
ok(leadHouseRemaining(colo26).value == null, 'colorado 2026 leftover stays empty — partial window does not invent leftover')
ok(colo26.nil.year1Lead?.booked?.value === 7_672_052.88, 'colorado 2026 year1Lead holds the partial window')
ok(nilBooked(colo26) == null, 'colorado 2026 nilBooked() stays pending so ratios do not rebook the partial as 2026–27')
...
ok(indexSrc.includes('<h1 class="issue-hed">What can your team actually afford?</h1>'), 'homepage hed is the locked slogan')
ok(!indexSrc.includes('Two ceilings, then booked NIL.'), 'homepage hed dropped the two-ceilings sentence')
ok(indexSrc.includes('Not total athletic revenue, and not a Group of 6 predictor'), 'homepage lede is in first HTML')
ok(indexSrc.includes('Seasons run 2021–2026 (NIL era).'), 'homepage lede keeps the NIL-era season span')
