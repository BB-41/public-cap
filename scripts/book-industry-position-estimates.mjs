/**
 * Surgically upsert nil.industryPositionEstimates. Does not rewrite the rest of schools.json.
 * Run: node scripts/book-industry-position-estimates.mjs
 */
...
const cells = JSON.parse(readFileSync(join(root, 'scripts/industry-position-estimates.json'), 'utf8'))
...
for (const sid of ids) {
  ...
  const dumped = JSON.stringify(cleaned, null, 2)
    ...
    .join('\n')
  const valueJson = `        "industryPositionEstimates": ${dumped}`
  const next = upsertKey(obj, 'industryPositionEstimates', valueJson)
...
if (after.lsu.nil.industryPositionEstimates) throw new Error('LSU must not book a position survey (no Seaton)')
if (after.alabama.nil.industryPositionEstimates) throw new Error('Alabama must stay empty')
if (after.texas.nil.houseRemaining.value !== 7_000_000) throw new Error('Texas leftover drifted')
if (!after.miami.nil.industryPositionEstimates?.positions?.some((p) => p.family === 'qb')) {
  throw new Error('Miami QB band missing')
}
if (JSON.stringify(after.miami.nil.industryPositionEstimates).toLowerCase().includes('seaton')) {
  throw new Error('position survey named Seaton')
}
if (Object.keys(cells).length !== upserted.length) throw new Error('upsert count mismatch')
console.log('upserted industryPositionEstimates for', upserted.sort().join(', '))
