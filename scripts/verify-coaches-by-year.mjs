...
ok(chair('south-carolina', 2026).pay.value === 8_250_000, 'South Carolina 2026 Beamer term-sheet $8.25M')
ok(!(chair('south-carolina', 2026).pay?.source || '').includes('USA TODAY'), 'South Carolina 2026 is not USA TODAY')
ok(chair('south-carolina', 2025).pay.value === 8_150_000, 'South Carolina 2025 stays USA TODAY $8.15M')
...
ok(byId.texas.nil?.booked?.value === 17_999_479.04, 'Texas booked NIL is the Jul 2025–Jun 2026 window')
...
ok((byId.missouri.coaches.football.buyout?.steps || []).length === 6, 'Missouri 80% × CY table is now a derived step tape')
...
ok(byId.texas.nil.houseRemaining?.value === 2_500_520.96, 'Texas remaining is cap minus window 1')
ok(byId.texas.nil.houseRemaining?.partialYear === false, 'Texas remaining is no longer the YTD hold')
